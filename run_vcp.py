...
def run_vcp(ind_params, model_ndx, vc_data, dt=0.1, protocol_name='vcp', rtrn=False):
    ...
    t_bounds = [(np.abs(vcp_response.t - 9990).argmin()),
                (len(vcp_response.t)-1)]
    t_simu = vcp_response.t[t_bounds[0]:t_bounds[1]] - 10000.0
    print('t_bounds: ', t_bounds)
    print('t_simu[0]: ', t_simu[0], ' t_simu[-1]: ', t_simu[-1])
    # Check bounds
